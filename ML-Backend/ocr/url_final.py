...
class EnhancedIntegratedProductPipeline:
    def __init__(self, csv_file_path: str, tavily_api_key: str = None):
        """
        Initialize the enhanced integrated pipeline
    ...
    def process_url_to_product_details(self, 
                                     product_url: str,
                                     additional_info: Dict = None) -> Dict:
        """
        Enhanced pipeline: URL -> Product Name -> Product Details with guaranteed ingredients
        
        Args:
            product_url: The product URL to extract name from
        ...
        try:
            ...
            print(f"Extracting product name from URL: {product_url}")
            ...
            extracted_weight = self._extract_weight_from_name(product_name)
            ...
            if not search_result.get('found', False) and extracted_brand != 'Unknown':
            ...
            if search_result.get('found', False):
                ...
                return {
                    ...
                    "success": False,
                    "search_method": "product_name_extracted_with_generated_ingredients",
                    "source_url": product_url,
                    "note": "Product name extracted but details not found in database. Ingredients generated based on product type."
                }
    ...
    def _simplify_product_name(self, product_name: str) -> str:
        ...
        # Remove common marketing phrases and extra details
        removal_patterns = [
    ...
    def batch_process_urls(self, urls_with_info: list) -> list:
        """
        Process multiple URLs at once
        """
        results = []
        
        for item in urls_with_info:
            if isinstance(item, str):
                result = self.process_url_to_product_details(item)
            elif isinstance(item, dict) and 'url' in item:
                url = item.pop('url')
                additional_info = item
                result = self.process_url_to_product_details(url, additional_info)
            else:
                result = {
...
def get_product_details_from_url(product_url: str, 
                                csv_file_path: str,
                                additional_info: Dict = None,
                                tavily_api_key: str = None) -> Dict:
    """
    Enhanced function to get complete product details from a URL with guaranteed ingredients
    """
    pipeline = EnhancedIntegratedProductPipeline(csv_file_path, tavily_api_key)
    return pipeline.process_url_to_product_details(product_url, additional_info)


# Example usage
def main():
    """
    Demo of the enhanced integrated pipeline with guaranteed ingredients
    """
    # Your CSV file path
    CSV_PATH = os.path.join(os.path.dirname(__file__), "merged_dataset.csv")
    
    # Initialize the enhanced pipeline
    pipeline = EnhancedIntegratedProductPipeline(CSV_PATH)
    
    # Example URLs to test
    test_urls = [
        "https://www.amazon.in/LAKM%C3%89-Absolute-Brightening-Illuminating-Moisturizer/dp/B01C571752/ref=sr_1_2_sspa?dib=eyJ2IjoiMSJ9.os8JecgCACXqT42-yaqorK8Hco2d4ffkfc60hVqXupDCeuRlP7qAHDPu8DbuMbV2XEloskIzBUcoS2wr06UaUCXfuL4Swk-FAMy3djionp4kPlqbVDgLu_ERoyHAAU3q135r90k9Az48tD-w5BUTdSC4eFtYKgNX7WWNYhbSD_MaRaDJ9uhAme64esd3GrNyO2zRnbNvwr7RiAjUsfqx51nmV0dAX_SiUhvI2QxBEkgkC5seYCOTnjsynEaLsMK7mMQyRFi12DdY6JQmQpm2adFvT55D_dCgl9kdM9JhH40.c5t_ct6YBSMu8CtflQ5WZPG9rgyKYKx8aYcO11QPbME&dib_tag=se&keywords=cream&qid=1757324927&sr=8-2-spons&sp_csd=d2lkZ2V0TmFtZT1zcF9hdGY&th=1",
        "https://www.amazon.in/Moisturizer-Non-Greasy-Vitamin-Instant-Hydration/dp/B00E96N6O8/ref=sr_1_1_sspa?dib=eyJ2IjoiMSJ9.os8JecgCACXqT42-yaqorK8Hco2d4ffkfc60hVqXupDCeuRlP7qAHDPu8DbuMbV2XEloskIzBUcoS2wr06UaUCXfuL4Swk-FAMy3djionp4kPlqbVDgLu_ERoyHAAU3q135r90k9Az48tD-w5BUTdSC4eFtYKgNX7WWNYhbSD_MaRaDJ9uhAme64esd3GrNyO2zRnbNvwr7RiAjUsfqx51nmV0dAX_SiUhvI2QxBEkgkC5seYCOTnjsynEaLsMK7mMQyRFi12DdY6JQmQpm2adFvT55D_dCgl9kdM9JhH40.c5t_ct6YBSMu8CtflQ5WZPG9rgyKYKx8aYcO11QPbME&dib_tag=se&keywords=cream&qid=1757324927&sr=8-1-spons&sp_csd=d2lkZ2V0TmFtZT1zcF9hdGY&th=1"
    ]
    
    # Test single URL processing
    print("=== ENHANCED SINGLE URL PROCESSING ===")
    ...
    result = pipeline.process_url_to_product_details(test_url, additional_info)
    
    print("Enhanced Result with Guaranteed Ingredients:")
